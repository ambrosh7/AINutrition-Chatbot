...
Railway injects `PORT` and must bind `0.0.0.0`. There is no `backend/railway.toml` yet. `web/vercel.json` still has a placeholder host.

### 1. Production start command (Railway)

Add `backend/railway.toml` **or** a root `railway.toml`. Prefer **repo root as the Railway service root** so `app.config.REPO_ROOT` (two levels above `backend/app/config.py`) still resolves to the monorepo root and `data/app.db` / `data/groq-usage.json` land in the expected place.
